...
def read_data(data_pos, data_neg, n):
    ...
    for i,line in enumerate(neg):
        a = line.split(' ') # tách các dòng thành các chuỗi con, cắt theo dấu space
        dat[i+len(pos),:] = np.array([int(a[0]),int(a[1]),int(a[2])]) # gán giá trị tương ứng vào dòng thứ i của ma trận dat
    ...
    data = coo_matrix((dat[:, 2], (dat[:, 0], dat[:, 1] - 1)), shape = (len(label),nwords))
    return (data,label)
